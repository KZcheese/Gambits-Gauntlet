...
public class MultiplayerGrabScript : MonoBehaviour
{
    //Adding Here
    ...
    void Update()
    {
        ...
        else if (movementType == MultiplayerPlayerController.MovementType.controller)
        {
        if (isLeftArm)
            {
                if (!Input.GetButton("LeftGamepad"))
                {
                    myAnimator.SetBool("HoldingLeft", false);
                    myAnimator.SetBool("Holding", false);
                    dropObject();
                ...
                {
                    if (Input.GetButton("RightGamepad"))
                    {
                        myAnimator.SetBool("Holding", true);
                    }
                    else
            ...
            else
            {
                if (Input.GetButton("RightGamepad"))
                {
                    if (Input.GetButtonDown("LeftGamepad"))
                    {
                        myAnimator.SetBool("Holding", true);
                    }
                    else
            ...
            if (Input.GetButtonUp("RightGamepad"))
            {
                Cursor.lockState = CursorLockMode.Locked;
            }
            //R2
            if (Input.GetButtonDown("FireGunGamepad"))
            {
                if (isHolding)
                {
                    if (heldObject.GetComponent<GunScript>() != null)
    ...
    void OnTriggerEnter(Collider other){
        ...
        else if (movementType == MultiplayerPlayerController.MovementType.controller)
        {
            if (isLeftArm)
            {
                if (Input.GetButton("LeftGamepad") && other.gameObject.GetComponent<Rigidbody>() != null && other.gameObject.tag != "Player" && (other.gameObject.tag == "grabbable" || other.gameObject.tag == "gun"))
                {
                    if (other.gameObject.tag == "gun")
                    {
                        other.gameObject.transform.position = gunHolder.position;
            ...
            {
                if (Input.GetButton("RightGamepad") && other.gameObject.GetComponent<Rigidbody>() != null && other.gameObject.tag != "Player" && (other.gameObject.tag == "grabbable" || other.gameObject.tag == "gun"))
                {
                    if (other.gameObject.tag == "gun")
                    {
                        other.gameObject.transform.position = gunHolder.position;
